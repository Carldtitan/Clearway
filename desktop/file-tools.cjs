...
function createFileTools({ emit, openPath, ocrCachePath }) {
  ...
  async function setRoots(filePaths) {
    ...
    activity(
      "completed",
      roots.size
        ? `Clearway can now search ${roots.size} approved ${roots.size === 1 ? "folder" : "folders"}.`
  ...
  async function execute(request) {
    if (!request || typeof request !== "object" || typeof request.tool !== "string") {
      throw new Error("Clearway received an invalid computer action.");
    }
  ...
  async function searchFiles(args) {
    if (!roots.size) throw new Error("Choose folders before asking Clearway to search.");
    const query = boundedString(args.query, 500);
    if (!query) throw new Error("Add something for Clearway to find.");
