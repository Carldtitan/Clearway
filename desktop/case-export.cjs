...
async function exportCaseFolder({ archive, applicantName, dialog, linkedFiles, mainWindow, missingDocuments, shell }) {
  ...
  if (formEntries.length < 5) {
    throw new Error("Clearway did not receive all five generated case documents.");
  }
  const selection = await dialog.showOpenDialog(mainWindow, {
    title: "Choose where Clearway should create the case folder",
  ...
  const baseName = `Clearway SSDI - ${safeName(applicantName || "Applicant")}`;
  ...
  const missingText = missing.length
    ...
    : "No supporting documents are currently marked missing in Clearway.\r\n";
  ...
  await fs.writeFile(
    ...
    [
      ...
      "",
      "Review every document and sign where required. Clearway does not submit this folder to SSA.",
    ].join("\r\n"),
    "utf8",
  );
...
function toUint8Array(value) {
  ...
  throw new Error("Clearway received an invalid forms archive.");
}
...
async function uniqueDirectory(parent, baseName) {
  ...
  throw new Error("Clearway could not choose a unique case-folder name.");
}
...
async function uniqueFileName(destination, requestedName) {
  ...
  throw new Error("Clearway could not choose a unique evidence filename.");
}
