...
public static class ClearwayNative {
...
function Get-ActiveWindow {
  $handle = [ClearwayNative]::GetForegroundWindow()
  if ($handle -eq [IntPtr]::Zero) { return $null }
  $element = [Windows.Automation.AutomationElement]::FromHandle($handle)
  $builder = New-Object Text.StringBuilder 512
  [void][ClearwayNative]::GetWindowText($handle, $builder, $builder.Capacity)
  [uint32]$processId = 0
  [void][ClearwayNative]::GetWindowThreadProcessId($handle, [ref]$processId)
  $processName = ""
...
function Invoke-Element($element) {
  ...
  [ClearwayNative]::SetCursorPos([int]($bounds.X + $bounds.Width / 2), [int]($bounds.Y + $bounds.Height / 2)) | Out-Null
  [ClearwayNative]::mouse_event(0x0002, 0, 0, 0, [UIntPtr]::Zero)
  [ClearwayNative]::mouse_event(0x0004, 0, 0, 0, [UIntPtr]::Zero)
}
...
function Perform-ComputerAction($tool, $args) {
  ...
  switch ($tool) {
    ...
    "click" {
      [ClearwayNative]::SetCursorPos([int]$args.x, [int]$args.y) | Out-Null
      [ClearwayNative]::mouse_event(0x0002, 0, 0, 0, [UIntPtr]::Zero)
      [ClearwayNative]::mouse_event(0x0004, 0, 0, 0, [UIntPtr]::Zero)
    }
    "type_text" {
      ...
      if (
        ...
        [ClearwayNative]::TypeUnicode([string]$args.text)
      }
    }
    "press_keys" {
      [uint16[]]$keys = @($args.keys | ForEach-Object { Key-Code ([string]$_) })
      [ClearwayNative]::PressChord($keys, [int]$args.repeats)
    }
    "scroll" {
      $delta = if ($args.direction -eq "up") { 120 * [int]$args.amount } else { -120 * [int]$args.amount }
      [ClearwayNative]::mouse_event(0x0800, 0, 0, $delta, [UIntPtr]::Zero)
    }
...
switch ($Action) {
  ...
  "Click" {
    [ClearwayNative]::SetCursorPos([int]$payload.x, [int]$payload.y) | Out-Null
    [ClearwayNative]::mouse_event(0x0002, 0, 0, 0, [UIntPtr]::Zero)
    [ClearwayNative]::mouse_event(0x0004, 0, 0, 0, [UIntPtr]::Zero)
    $result = @{ ok = $true }
  }
  "TypeText" {
    ...
    if (
      ...
      [ClearwayNative]::TypeUnicode([string]$payload.text)
    }
    $result = @{ ok = $true }
  }
  "PressKeys" {
    [uint16[]]$keys = @($payload.keys | ForEach-Object { Key-Code ([string]$_) })
    [ClearwayNative]::PressChord($keys, [int]$payload.repeats)
    $result = @{ ok = $true }
  }
  "Scroll" {
