...
### 5. V1 deterministic demo handoff

1. The Application header exposes a secondary demo-fill action after language selection.
2. A confirmation dialog names the destructive scope before any case replacement.
3. Confirmation increments the conversation run identifier, cancels active recording and playback, and dispatches one `LOAD_CASE` action with a fresh clone of the Synthetic_Applicant.
4. The replacement path never applies field-level merge actions, so original facts, transcripts, conflicts, deferred questions, and provenance cannot leak into the demo case.
5. The selected Conversation_Locale is applied to the cloned demo case before the atomic reducer action.
6. The loaded state identifies itself as synthetic, summarizes the evidence-rich fixture, and exposes one primary transition to Documents.
7. The fixture includes a prior full-duty job, a current accommodated part-time job, two conditions, three treatment sources, two medications, diagnostic testing, a third-party contact, and mixed record-request states.

### 6. V1 records demonstration
